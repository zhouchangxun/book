...
var disqusCode = '<h3>留言</h3><div id="disqus_thread"> 还没有留言。</div>';
...
function searchbar_listener(event) {
    ...
    if (q !== '') {
      var url = 'https://github.com/someone/someRepo/search?utf8=✓&q=' + encodeURIComponent(q);
...
function router() {
  ...
  console.log('loading new content: ',path);
  ...
  $.get(path, function(data) {
    console.log('[update] fetch data finish.');

    $(ditto.error_id).hide();

    console.log(getDate()+'[render] origin md to html...');
    ...
    console.log(getDate()+'[render] finish...');
    ...
    function load_disqus() {
      console.log('loading disqus data');
...
function initialize() {
  console.log('[init] initialize sidebar section...');
  if (ditto.sidebar) {
  ...
  // add url hashchange event listener. it can be called when path after '#' changed.
  console.log('[init] registe url change event.');
  $(window).on('hashchange', router);

  console.log('[init] book system ready.');

  // we invoke it munually at first.
  console.log('[init] fetch first content.');
  router();
}
/* end */